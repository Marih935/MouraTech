...
function media_idade(Alunos) {
  ...
  for (let i = 0; i < Alunos.length; i++) {
    let data = Alunos[i].nascimento.split("/");
    let diaNasc = data[0];
    let mesNac = data[1];
    let anoNasc = data[2];
    let dataAtual = new Date();
    let diaAtual = dataAtual.getDate();
    let mesAtual = dataAtual.getMonth() + 1; //+1 pois no getMonth Janeiro começa com zero.
    let anoAtual = dataAtual.getFullYear(2023);
    let idade = anoAtual - anoNasc;
    if (mesAtual < mesNac || mesAtual == mesNac && diaAtual < diaNasc) {
      idade--; //Se a data atual for menor que a data de nascimento ele não faz aniversário.
    } else {
      idade;
    }
    soma += idade;
...
console.log(media_idade(Alunos));